...
- [x] Phase 8: Ratings, Reviews, Wishlist & Advanced Search
  ...
  - [x] Phase 8.4: Machinery Rental Marketplace (Redesigned Machinery entities to ApplicationUser level for dual Farmer/Customer ownership & renting; EF Core migration AddMachineryRentalUserBased applied; direct booking lifecycle: Booked → Confirmed → ReadyForHandover → RentedOut → Returned → Completed (+ Cancelled); backend date overlap validation, self-rental prevention, mock payment integration, in-app notifications; Machinery & Rentals APIs; Machinery Angular service & models; Customer Machinery Marketplace at /customer/machinery, detail & date picker at /customer/machinery/:id, my rentals at /customer/my-rentals; Farmer Machinery management at /farmer/machinery, create/edit form at /farmer/machinery/new & /farmer/machinery/:id/edit, photo upload gallery up to 5 images, incoming rental booking pipeline management at /farmer/machinery/rentals; Wishlist integration for Machinery type; navigation links added to both shells; full backend & frontend builds passing with 0 errors)

...
public sealed class FarmerAuctionService(FarmKartDbContext dbContext) : IFarmerAuctionService
{
    ...
    {
        var farmer = await GetFarmerAsync(userId, cancellationToken);
        ValidateSchedule(request.StartTimeUtc, request.EndTimeUtc);
        ...
        var auction = new Auction { CropListing = listing, FarmerProfileId = farmer.Id, StartingPrice = request.StartingBidPrice, CurrentHighestBid = 0m, MinimumBidIncrement = request.MinimumBidIncrement, StartTimeUtc = request.StartTimeUtc, EndTimeUtc = request.EndTimeUtc, AuctionStatus = status };
    ...
    public async Task<FarmerAuctionResponse> UpdateAuctionAsync(Guid userId, Guid auctionId, UpdateFarmerAuctionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        auction.StartingPrice = request.StartingBidPrice; auction.MinimumBidIncrement = request.MinimumBidIncrement; auction.StartTimeUtc = request.StartTimeUtc; auction.EndTimeUtc = request.EndTimeUtc;
    ...
    private async Task EnsureAvailableAsync(Crop crop, decimal quantity, MeasurementUnit unit, Guid? excludedAuctionId, CancellationToken ct)
    {
        ...
        var reservedKg = await dbContext.Auctions.Include(a => a.CropListing)
            .Where(a => a.CropListing.CropId == crop.Id && ReservingStatuses.Contains(a.AuctionStatus) && (!excludedAuctionId.HasValue || a.Id != excludedAuctionId.Value))
    ...
    {
        if (start < DateTime.UtcNow) throw new ArgumentException("Auction start time must be now or in the future.");
    ...
    private async Task<FarmerAuctionResponse> MapAsync(Auction auction, CancellationToken ct)
    {
        ...
        var reserved = await dbContext.Auctions.Include(a => a.CropListing).Where(a => a.CropListing.CropId == auction.CropListing.CropId && ReservingStatuses.Contains(a.AuctionStatus)).SumAsync(a => (decimal?)a.CropListing.QuantityForSale * (a.CropListing.Unit == MeasurementUnit.Ton ? 1000m : a.CropListing.Unit == MeasurementUnit.Quintal ? 100m : 1m), ct) ?? 0m;
        var kg = CropStockUnitConverter.ToKilograms(auction.CropListing.QuantityForSale, auction.CropListing.Unit);
        return new(auction.Id, auction.CropListing.CropId, auction.CropListing.Crop.CropName, auction.CropListing.QuantityForSale, CropStockUnitConverter.Format(auction.CropListing.Unit), kg, total, reserved, total - reserved, auction.StartingPrice, auction.MinimumBidIncrement, auction.StartTimeUtc, auction.EndTimeUtc, auction.AuctionStatus.ToString(), auction.CropListing.Description, auction.CreatedAtUtc, auction.UpdatedAtUtc);
    }
}
